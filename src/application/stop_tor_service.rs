...
impl StopTorService {
    ...
    pub async fn execute(&self, service_name: &str) -> Result<()> {
        ...
        // Services can be named directly or with enola_ prefix (for Tor directory organization)
        let names_to_try = if service_name.starts_with("enola_") {
            vec![service_name.to_string()]
        } else {
            vec![service_name.to_string(), format!("enola_{}", service_name)]
        };

        let mut last_error = None;
        let mut tor_stopped = false;
        let mut actual_name = String::new();

        // 1. Try to stop Tor service with each possible name
        for name in &names_to_try {
            match self.tor_manager.stop_hidden_service(name).await {
                Ok(_) => {
                    tor_stopped = true;
        ...
        // 2. Try to disable Nginx site (only for proxy services)
        if let Some(nginx) = &self.nginx_manager {
            // Try with the actual name that worked for Tor
            if nginx.disable_site(&actual_name).await.is_ok() {
...
mod tests {
    ...
    impl TorManagerPort for MockTorManager {
        ...
        async fn stop_hidden_service(&self, _: &str) -> Result<()> {
            ...
            if self.should_fail {
                Err(EnolaError::InfrastructureError("Stop failed".into()))
            } else {
                Ok(())
            }
        }
    ...
    impl MockNginxManager {
        fn new() -> Self {
            Self {
                disable_called: AtomicBool::new(false),
            }
        }
    }
    ...
    #[tokio::test]
    async fn test_stop_tor_service_success() {
        let tor = Arc::new(MockTorManager::new());
        let use_case = StopTorService::new(tor.clone(), None);

        let result = use_case.execute("myservice").await;

        assert!(result.is_ok());
        assert!(tor.stop_called.load(Ordering::SeqCst));
    }

    #[tokio::test]
    async fn test_stop_tor_service_with_nginx() {
        let tor = Arc::new(MockTorManager::new());
        let nginx = Arc::new(MockNginxManager::new());
        let use_case = StopTorService::new(tor.clone(), Some(nginx.clone()));

        let result = use_case.execute("myservice").await;

        assert!(result.is_ok());
        assert!(tor.stop_called.load(Ordering::SeqCst));
        assert!(nginx.disable_called.load(Ordering::SeqCst));
    }

    #[tokio::test]
    async fn test_stop_tor_service_tor_failure() {
        let tor = Arc::new(MockTorManager::failing());
        let use_case = StopTorService::new(tor, None);
